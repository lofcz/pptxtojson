...
function convertEOTtoTTF(eotBuffer) {
  ...
  try {
    ...
    if (ttfBytes.length >= 4) {
      const ttfMagic = (ttfBytes[0] << 24) | (ttfBytes[1] << 16) | (ttfBytes[2] << 8) | ttfBytes[3]
...
export async function getFontData(zip, fontPath) {
  ...
  try {
    ...
    if (!fontFilename.toLowerCase().endsWith('.fntdata') && !fontFilename.toLowerCase().endsWith('.eot')) {
      return ''
    }
...
const SYSTEM_FONT_FALLBACK_MAP = {
  ...
  'Microsoft YaHei UI': 'Microsoft YaHei UI, Microsoft YaHei, PingFang SC, sans-serif',
  ...
  'Hiragino Kaku Gothic Pro': 'Hiragino Kaku Gothic Pro, Microsoft YaHei, sans-serif',
